...
inline std::vector<float> IntersectRaySphere( vec3 &origin, const vec3 &direction, const sphere &s) {
...
inline std::tuple<sphere, float> ClosestIntersection(Scene &s, vec3 &origin, vec3 &direction, float min_t, float max_t) {
...
inline float ComputeLighting(Scene &s, vec3 &point, vec3 &normal, vec3 direction, float specular, float max_t) {
    float i = 0;
    float dot;
    vec3 l;
...
inline color TraceRay(Scene &s, vec3 &origin, vec3 &direction, float min_t, float max_t, int recursionDepth) {
    ...
    color reflectColor = TraceRay(s, intersect_point, reflectDir, 0.01, max_t, recursionDepth +1);
